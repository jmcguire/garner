# Irreversible Binomials

Irreversible Binomials are idiomatically unalterable verbal pairings—grammatically identical terms joined usually by a conjunction and customarily placed in a particular order so that inverting them would seem surprising and quirky. Everybody says *rank and file*, not \**file and rank*. As a hardened collocation, an irreversible binomial is a type of set phrase.

The terms in a binomial are typically antonyms, synonyms, or complementary words that are so conventionally joined in a specific sequence as to seem, to speakers of a given language, inevitably paired that way. In terms of frequency in one order as opposed to the other, a 3:1 ratio isn't enough for irreversibility. *Books and magazines*, for example, occurs more often than *magazines and books*—but 25% of the time the latter phrasing occurs. The same is true of *shoes and socks* vs. *socks and shoes*. The tendency must be much stronger for irreversibility: with *cops and robbers* the ratio is 318:1 over \**robbers and cops*.

Sometimes the binomials are alliterative <flora and fauna> or even rhyming <slice and dice>, but perhaps less often than one might suspect. Often they're chronologically ordered <come and go> <wax and wane>. The positive concept usually precedes the negative <friend or foe> <good and bad>. Proximity is usually given precedence over distance <here and there> <this and that>. Some are clichés <time and tide> <whys and wherefores>.

The terms in a binomial might be any part of speech, but most often they're nouns, verbs, or adjectives. In many pairings of complementary nouns, foodstuffs that commonly go together are yoked into a phrase, the more substantial item normally coming first <cookies and cream> <macaroni and cheese>. Various categories might be created, but these are perhaps the most typical semantic groupings:

Antonyms or Contrasting Words

 - back and forth
 - balls and strikes
 - better or worse
 - black and white
 - the birds and the bees
 - cats and dogs
 - the chicken or the egg
 - come and go
 - confirm or deny
 - cops and robbers
 - costs and benefits
 - dead or alive
 - do or die
 - flesh and bone
 - flora and fauna
 - friend or foe
 - good and bad
 - heaven and earth
 - heaven and hell
 - here and there
 - hide and seek
 - high and low
 - hot and cold
 - ins and outs
 - land and sea
 - lost and found
 - more or less
 - night and day
 - now and forever
 - now and then
 - oil and water
 - peaks and valleys
 - profits and losses
 - pros and cons
 - rain or shine
 - richer or poorer
 - right and wrong
 - shirts and skins
 - sink or swim
 - surf and turf
 - sweet and savory
 - sweet and sour
 - thick and thin
 - to and fro
 - trick or treat
 - true or false
 - ups and downs
 - wax and wane
 - with or without
 - yin and yang

Synonyms and Near-Synonyms

 - aches and pains
 - armed and dangerous
 - beck and call
 - bits and pieces
 - cease and desist
 - clear and convincing
 - down and out
 - dribs and drabs
 - drunk and disorderly
 - fear and loathing
 - far and away
 - first and foremost
 - flotsam and jetsam
 - fun and games
 - heart and soul
 - hem and haw
 - high and mighty
 - hopes and dreams
 - hot and bothered
 - house and home \[eat out of\]
 - huffing and puffing
 - hustle and bustle
 - intents and purposes
 - kit and caboodle
 - kith and kin
 - leaps and bounds
 - lo and behold
 - love and cherish
 - metes and bounds
 - mortar and pestle
 - near and dear
 - nook and cranny
 - null and void
 - oohs and ahs
 - out and about
 - over and above
 - over and out
 - peace and quiet
 - pick and choose
 - plain and simple
 - rack and ruin
 - rest and relaxation
 - safe and sound
 - separate and apart
 - sick and tired
 - slice and dice
 - spick and span
 - terms and conditions
 - tried and true
 - ways and means
 - whys and wherefores

Complementary Words

 - above and beyond
 - alpha and omega
 - apples and oranges
 - bacon and eggs
 - bed and breakfast
 - bells and whistles
 - belt and suspenders
 - block and tackle
 - blood and guts
 - body and soul
 - bow and arrow
 - bread and butter
 - brick and mortar
 - burger and fries
 - cloak and dagger
 - cookies and cream
 - cream and sugar
 - cut and dried
 - cut and paste
 - death and taxes
 - divide and conquer
 - dollars and cents
 - early and often
 - eyes and ears
 - fair and square
 - fame and fortune
 - fast and furious
 - fat and happy
 - fire and brimstone
 - ham and cheese
 - hammer and nail
 - hearts and minds
 - highways and byways
 - horse and buggy
 - live and learn
 - lock and key
 - macaroni and cheese
 - milk and cookies
 - needle and thread
 - nickel-and-dime
 - nip and tuck
 - nuts and bolts
 - older and wiser
 - one and all
 - p's and q's
 - peaches and cream
 - rank and file
 - research and development
 - rise and shine
 - salt and pepper
 - shake and bake
 - shipping and handling
 - short and sweet
 - skin and bones
 - song and dance
 - spaghetti and meatballs
 - spit and image
 - sticks and stones
 - stocks and bonds
 - sugar and spice
 - tooth and claw
 - today and tomorrow
 - wear and tear
 - wine and dine

Some writers intentionally invert the word order as a poetic device. For example, in Billy Joel's song "Piano Man," the old man sitting next to the narrator is "making love to his tonic and gin" \(as opposed to the expected *gin and tonic*, the reversal in order perhaps suggesting inebriation—not to mention supplying the lyricist's need for a rhyme with the word *in*\). With ordinary prose, however, writers just come off as eccentric or disturbed if they write \**ends and odds* or \**parcel and part*. Such anomalous examples can almost always be found: "If a sufficiently large corpus is consulted, no binomial will attain the status of frozenness." Sandra Mollin, *The \(Ir\)reversibility of English Binomials* 43 \(2014\). But the well-known phrases are often irreversible 99% of the time in corpus data. *Ibid*. at 41. Some writers will actually use \**forth and back*, but anyone doing so should have a particular reason.

At least one writer has called inverted binomials "ungrammatical." See Earl R. Anderson, *A Grammar of Iconism* 267 \(1998\) \(mistakenly calling \**Eve and Adam*, \**that and this*, and \**there and here* "ungrammatical"\). These phrases work grammatically, but they're entirely unidiomatic. What the writer meant to say, perhaps, is "unacceptable" or "wrong." But the problem has nothing to do with grammar.

Not all binomials involve conjunctions. Some are reduplicative terms <ding-dong> <easy peasy> <wishy-washy>. Others contain prepositions <cheek by jowl> <pillar to post> <tit for tat>. Other examples of these types will readily come to mind.

*Trinomials* are phrases with three related words. \(Some writers use the term *multinomials* for three or more.\) Some trinomials—like *ready, aim, fire* and *veni, vidi, vici—*use asyndeton, without a conjunction. But most use *and*, and a few use *or*. These are typical examples:

 - bacon, lettuce, and tomato
 - beg, borrow, or steal
 - bell, book, and candle
 - blood, sweat, and tears
 - cool, calm, and collected
 - duck, bob, and weave
 - eat, drink, and be merry
 - faith, hope, and charity
 - first, last, and always
 - friends, Romans, countrymen
 - game, set, and match
 - good, bad, or indifferent
 - the good, the bad, and the ugly
 - here, there, and everywhere
 - hither, thither, and yon
 - hook, line, and sinker
 - hop, skip, and jump
 - ifs, ands, or buts
 - judge, jury, and executioner
 - land, sea, and air
 - lather, rinse, and repeat
 - left, right, and center
 - liberty, equality, fraternity
 - life, liberty, and the pursuit of happiness
 - lights, camera, action
 - live, laugh, love
 - lock, stock, and barrel
 - me, myself, and I
 - mind, body, and soul
 - morning, noon, and night
 - past, present, and future
 - person, place, or thing
 - punt, pass, and kick
 - reading, writing, and arithmetic
 - ready, set, go
 - ready, willing, and able
 - red, white, and blue
 - sex, drugs, and rock 'n' roll
 - signed, sealed, and delivered
 - snap, crackle, and pop
 - stop, drop, and roll
 - stop, look, and listen
 - tall, dark, and handsome
 - this, that, and the other
 - Tom, Dick, and Harry
 - the truth, the whole truth, and nothing but the truth
 - tic-tac-toe
 - way, shape, or form
 - win, lose, or draw
 - win, place, or show

To be more accurate, trinomials like these might be called *unalterable*, *unchangeable*, or perhaps *unshufflable* instead of *irreversible*, given that reversals occur with twos, not threes.

A 2018 "Non Sequitur" cartoon by Wiley Miller shows six people at a burial. An elderly woman, looking down at the casket, says, "I kept telling him he had it backwards. Needless to say, he was a pretty slow learner." On the tombstone is inscribed this motto: "Learn and Live."

