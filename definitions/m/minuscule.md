# minuscule

minuscule. So spelled, not *\*miniscule*. The word derives from the word *minus*; it has nothing to do with the prefix *mini*-. But the misspelling has spread since about 1950—e.g.:

 - "Mouth hanging open, Harry saw that the little square for June thirteenth seemed to have turned into a *miniscule* \[read *minuscule*\] television screen." J.K. Rowling, *Harry Potter and the Chamber of Secrets* 242 \(Am. ed. 1999\).

 - "Even as some people questioned the practical effect of saving such a *miniscule* \[read *minuscule*\] portion of the state budget, they were mostly willing to forgo cynicism." Kathleen Burge, "Forgoing of Salaries Gets Mixed Reviews," *Boston Globe*, 2 Jan. 2003, at B5.

 - "Consider Iowa, with only a *miniscule* \[read *minuscule*\] African-American population." Abigail Thernstrom & Stephen Thernstrom, "Racial Gerrymandering Is Unnecessary," *Wall Street J.*, 11 Nov. 2008, at A20.

See spelling \(a\). Even so, the correct spelling has always vastly predominated in print sources.

The counterpart—a rarity—is *majuscule*. Today that term is used only in printing, to denote a capital letter.

In both AmE and BrE, *minuscule* is now pronounced /**min**-ә-skyool/—though before the 1960s the standard pronunciation stressed the second syllable: /mi-**nәs**-kyool/. Avoid /**min**-yә-skyool/.

## Language-Change Index

*minuscule* misspelled *\*miniscule*: Stage 4
Current ratio in print \(*minuscule* vs. \**miniscule*\): 1.6:1

